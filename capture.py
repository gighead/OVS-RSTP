...
def trigger_convergence(info_dict, user, password):
    '''
    Execute appropriate convergence commands on each nodes
    :return:
    '''
    ...
    for i in range(0, len(info_dict)):
        ...
        stdin, stdout, stderr = ssh[i].exec_command('nohup ~/./heart.sh > rstp.txt &')
        print stdout.readlines()
        print stderr.readlines()
        print 'tcpdump command is running'
...
def collect_info(info_file, user, code_dir):
    ...
    for node in my_dict:
        ...
        scp_cmd = "scp -i ~/.ssh/id_geni_ssh_rsa -P {0} -r {1} {2}@{3}:/users/{4}".format(conn_info[1], code_dir, user,
                                                                                          conn_info[0], user)
        # print scp_cmd
...
if __name__ == "__main__":
    ...
    uname = 'ss997901'
    code_dir = '/Users/SVP/Documents/Capstone-OVS/OVS-Automation/heart.sh'  # raw_input("Enter the code directory:")
    info_dict = collect_info(info_file, uname, code_dir)
    trigger_convergence(info_dict, uname, password='citrix123')
